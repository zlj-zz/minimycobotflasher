...
# timeout for starting flash (may perform erase)
...
class ESPLoader(object):
        ...
        try:
            ...
            for retry in range(100):
                ...
                if byte(data, 0) != 0 and byte(data, 1) == self.ROM_INVALID_RECV_MSG:
                    # Unsupported read_reg can result in more than one error response for some reason
                    self.flush_input()
    ...
    def write_reg(self, addr, value, mask=0xFFFFFFFF, delay_us=0, delay_after_us=0):
        ...
        if delay_after_us > 0:
            ...
            command += struct.pack('<IIII',
                                   self.UART_DATE_REG_ADDR, 0, 0, delay_after_us)
    ...
    def erase_region(self, offset, size):
        ...
        self.check_command("erase region", self.ESP_ERASE_REGION, struct.pack(
            '<II', offset, size), timeout=timeout)
    ...
    def read_flash(self, offset, length, progress_fn=None):
        if not self.IS_STUB:
            # ROM-only routine
            return self.read_flash_slow(offset, length, progress_fn)
    ...
    def get_crystal_freq(self):
        ...
        est_xtal = (self._port.baudrate * uart_div) / \
            1e6 / self.XTAL_CLK_DIVIDER
    ...
    def save(self, filename):
        ...
        with io.BytesIO() as f:  # write file to memory first
            ...
            def get_alignment_data_needed(segment):
                ...
                align_past = (segment.addr % self.IROM_ALIGN) - \
                    self.SEG_HEADER_LEN
                pad_len = (self.IROM_ALIGN - (f.tell() %
                                              self.IROM_ALIGN)) + align_past
            ...
            while len(flash_segments) > 0:
                ...
                if pad_len > 0:  # need to pad
                    ...
                    assert (
                        f.tell() + 8) % self.IROM_ALIGN == segment.addr % self.IROM_ALIGN
            ...
            if self.secure_pad:
                ...
                pad_len = (self.IROM_ALIGN - align_past -
                           checksum_space - space_after_checksum) % self.IROM_ALIGN
            ...
            if self.secure_pad:
                assert ((image_length + space_after_checksum) %
                        self.IROM_ALIGN) == 0
...
class ELFFile(object):
    ...
    def _read_sections(self, f, section_header_offs, section_header_count, shstrndx):
        ...
        prog_sections = [s for s in all_sections if s[1]
                         == ELFFile.SEC_TYPE_PROGBITS]
...
def slip_reader(port, trace_function):
    ...
    while True:
        ...
        trace_function("Read %d bytes: %s", len(
            read_bytes), HexFormatter(read_bytes))
        for b in read_bytes:
            if type(b) is int:
            ...
            if partial_packet is None:  # waiting for packet header
                if b == b'\xc0':
                    ...
                    trace_function("Read invalid data: %s",
                                   HexFormatter(read_bytes))
                    trace_function("Remaining data in serial buffer: %s", HexFormatter(
                        port.read(port.inWaiting())))
                    raise FatalError(
                        'Invalid head of packet (0x%s)' % hexify(b))
                ...
                if b == b'\xdc':
                    ...
                    trace_function("Remaining data in serial buffer: %s", HexFormatter(
                        port.read(port.inWaiting())))
                    raise FatalError(
                        'Invalid SLIP escape (0xdb, 0x%s)' % (hexify(b)))
            elif b == b'\xdb':  # start of escape sequence
                in_escape = True
            elif b == b'\xc0':  # end of packet
                trace_function("Received full packet: %s",
...
def write_flash(esp, args):
    ...
    if args.encrypt or args.encrypt_files is not None:
        ...
        if not esp.secure_download_mode:
            ...
            if crypt_cfg_efuse is not None and crypt_cfg_efuse != 0xF:
                print('Unexpected FLASH_CRYPT_CONFIG value: 0x%x' %
                      (crypt_cfg_efuse))
                do_write = False
    ...
    if args.encrypt_files is not None:
        ...
        # let's use sorted.
        all_files = sorted(all_files + encrypted_files_flag,
                           key=lambda x: x[0])
    ...
    for address, argfile, encrypted in all_files:
        ...
        while len(image) > 0:
            print_overwrite('Writing at 0x%08x... (%d %%)' % (
                address + seq * esp.FLASH_WRITE_SIZE, 100 * (seq + 1) // blocks))
            sys.stdout.flush()
            block = image[0:esp.FLASH_WRITE_SIZE]
            if compress:
                esp.flash_defl_block(
                    block, seq, timeout=DEFAULT_TIMEOUT * ratio * 2)
            else:
                # Pad the last block
        ...
        if compress:
            if t > 0.0:
                speed_msg = " (effective %.1f kbit/s)" % (uncsize /
                                                          t * 8 / 1000)
            print_overwrite('Wrote %d bytes (%d compressed) at 0x%08x in %.1f seconds%s...' % (
                uncsize, written, address, t, speed_msg), last_line=True)
        else:
            if t > 0.0:
                speed_msg = " (%.1f kbit/s)" % (written / t * 8 / 1000)
            print_overwrite('Wrote %d bytes at 0x%08x in %.1f seconds%s...' % (
                written, address, t, speed_msg), last_line=True)
...
def image_info(args):
    ...
    for seg in image.segments:
        idx += 1
        seg_name = ", ".join(
            [seg_range[2] for seg_range in image.ROM_LOADER.MEMORY_MAP if seg_range[0] <= seg.addr < seg_range[1]])
...
def main(custom_commandline=None):
    ...
    subparsers.add_parser('get_security_info',
                          help='Get some security-related data')
